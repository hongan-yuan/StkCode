...
from .constellation import orbit_plane, same_orbit
from .routing import route_data
from .service import Microservice, deployment_matrix
...
class ReplicaPlacementMigrationAgent:
    ...
    def estimate_service_pressure(self, requests) -> Counter:
        service_pressure = Counter()
        for request in requests:
            service_pressure.update(request.services)
    ...
    def apply(
        ...
        service_pressure = self.estimate_service_pressure(requests)
        ...
        for candidate in self._rank_candidates(candidates, microservices):
    ...
    def _candidate_arms(
        ...
        for service_id, _ in service_pressure.most_common(self.config.num_microservices):
            service = microservices[service_id]
    ...
    def _expected_saving(self, action: str, service_id: int, service_pressure: Counter) -> float:
        base = service_pressure[service_id] * 0.1
        if action == "add":
            return base
        if action == "move":
            return base * 0.75
