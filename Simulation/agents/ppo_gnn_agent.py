...
class PPOGNNExecutionAgent(ServiceExecutionAgent):
    ...
    def __init__(
        ...
        self.pending_episode_transitions: list[PPOTransition] = []
        self.rollout_buffer_size = max(1, int(config.ppo_rollout_buffer_size))
        self.device = None
        self.policy = None
        self.optimizer = None
    ...
    def record_terminal_outcome(
        ...
        if not self.training_available or not self.pending_episode_transitions:
            return
        if not math.isfinite(float(terminal_reward)):
            terminal_reward = -self.config.failure_penalty
        terminal_signal = float(terminal_reward) * self.config.ppo_terminal_reward_scale
        clip = max(0.0, float(self.config.ppo_terminal_reward_clip))
        if clip > 0.0:
            terminal_signal = max(-clip, min(clip, terminal_signal))

        episode = self.pending_episode_transitions
        episode[-1].done = True
        length = max(1, int(chain_length or len(episode)))
        length_divisor = length ** max(0.0, float(self.config.reward_chain_length_alpha))
        for index, transition in enumerate(episode):
            distance_to_terminal = len(episode) - index - 1
            discounted_share = (
                    self.config.ppo_terminal_reward_weight
                    * (self.config.ppo_gamma ** distance_to_terminal)
                    * terminal_signal
                    / length_divisor
            )
            transition.terminal_reward_share += discounted_share
            transition.reward = transition.local_reward + transition.terminal_reward_share
        self.pending_episode_transitions = []

    def clear_pending_episode(self) -> None:
        self.pending_episode_transitions = []

    def set_rollout_buffer_size(self, rollout_buffer_size: int) -> None:
        self.rollout_buffer_size = max(1, int(rollout_buffer_size))
        self._trim_transition_buffer()

    def _trim_transition_buffer(self) -> None:
        overflow = len(self.transitions) - self.rollout_buffer_size
        if overflow > 0:
    ...
    def _gae_advantages_and_targets(
            self,
        ...
        values = [
            float(transition.old_value.detach().cpu().reshape(()))
            for transition in transitions
        ]
        advantages = [0.0 for _ in transitions]
        targets = [0.0 for _ in transitions]
        running_gae = 0.0
        for index in range(len(transitions) - 1, -1, -1):
            transition = transitions[index]
            nonterminal = 0.0 if transition.done else 1.0
            next_value = values[index + 1] if index + 1 < len(transitions) else 0.0
            delta = transition.reward + gamma * next_value * nonterminal - values[index]
            running_gae = delta + gamma * gae_lambda * nonterminal * running_gae
            advantages[index] = running_gae
            targets[index] = running_gae + values[index]
    ...
    def ppo_update(
        ...
        gae_lambda = self.config.ppo_gae_lambda if gae_lambda is None else gae_lambda
        gae_lambda = max(0.0, min(1.0, float(gae_lambda)))
        advantages_all, value_targets_all = self._gae_advantages_and_targets(
            transition_pool, gamma, gae_lambda
        )
        ...
        advantages = torch.tensor(
            [advantages_all[index] for index in sampled_indices],
            dtype=torch.float32,
            device=self.device,
        )
        value_targets_t = torch.tensor(
            [value_targets_all[index] for index in sampled_indices],
            dtype=torch.float32,
            device=self.device,
        )
        ...
        for _ in range(epochs):
            ...
            if self.config.ppo_normalize_value_targets and value_targets_t.numel() > 1:
                target_mean = value_targets_t.mean().detach()
                target_std = value_targets_t.std().detach() + 1.0e-8
                value_loss = F.mse_loss(
                    (values - target_mean) / target_std,
                    (value_targets_t - target_mean) / target_std,
                )
            else:
                value_loss = F.mse_loss(values, value_targets_t)
            ...
            last_stats = {
                ...
                "value_target_mean": float(value_targets_t.mean().detach().cpu()),
                "value_target_std": (
                    float(value_targets_t.std().detach().cpu())
                    if value_targets_t.numel() > 1 else 0.0
                ),
            }
        return last_stats
