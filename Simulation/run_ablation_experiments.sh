...
RUN_ABLATIONS="${RUN_ABLATIONS:-${ABLATIONS:-SECO SP-Routing SC-NFV}}"
#MERGE_ABLATIONS="${MERGE_ABLATIONS:-ELARA ELARA-NB ELARA-NR ELARA-SH SECO SP-Routing SC-NFV}"
MERGE_ABLATIONS="${MERGE_ABLATIONS:-SECO SP-Routing SC-NFV}"
GPUS="${GPUS:-0}"
...
task_index=0
failed=0
total_tasks=$((${#RUN_ABLATION_ARRAY[@]} * ${#SEED_ARRAY[@]}))
...
write_rows(output_root / "all_ablation_cycle_request_metrics.csv", cycle_request_rows)
