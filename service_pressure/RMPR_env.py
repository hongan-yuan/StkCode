...
class RMPR:
        ...
        for source in nodes:
            for target in nodes:
                if source == target:
                    ...
                    try:
                        distance = nx.shortest_path_length(self.constellation, source=source, target=target,
                                                           weight=lambda x, y, z: 1 + self.get_satellite_by_id(
                                                               y).total_queue_length / self.get_satellite_by_id(
                                                               y).trans_throughput)
    ...
    def send_packets_to_src(self, flow, lambda_rate, static=False, info=None):
        ...
        if static:
            ...
            num = int(c_avg * 4)
            for i in range(num):
                packet = Packet(flow, [], flow.root, 0, info)
    ...
    def step(self, static=False):
        ...
        rates = {1: np.random.poisson(100), 2: np.random.poisson(100), 3: np.random.poisson(100), 4: np.random.poisson(100)}
        satellites_for_flows, routes = determine_routing_by_RMPR(self.all_flows, rates, self.constellation, self.deployment, self.all_satellites)
        input = 0
        for flow in self.all_flows:
            self.send_packets_to_src(flow, rates[flow.id], static, (satellites_for_flows[flow.id], routes[flow.id]))
            input += rates[flow.id]
        self.flow_input.append(input)

        for satellite in self.all_satellites:
            satellite.receive_packets_rmpr()
            satellite.increment_life_time_rmpr()
            satellite.step_c_rmpr()

        for satellite in self.all_satellites:
            satellite.step_t_rmpr()
...
if __name__ == "__main__":
    ...
    env = RMPR(deployment, constellation)
    for i in range(4):
    ...
    while t < 10000:
        ...
        if (t + 1) % 100 == 0:
            ...
            for satellite in env.all_satellites:
                print(f"satellite {satellite.id}: {satellite.queue_length()}")
    ...
    print(f'output: {np.mean(env.flow_output)}, input: {np.mean(env.flow_input)}, deliver rate: {np.mean(env.flow_output) / np.mean(env.flow_input)}')
