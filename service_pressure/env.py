...
class Env:
        ...
        for source in nodes:
            for target in nodes:
                if source == target:
                    ...
                    try:
                        distance = nx.shortest_path_length(self.constellation, source=source, target=target, weight=lambda x, y, z: 1 + self.get_satellite_by_id(y).total_queue_length /self.get_satellite_by_id(y).trans_throughput)
    ...
    def send_packets_to_src(self, flow, rate, static=False):
        # 向源卫星注入流量
        ...
        if static:
            ...
            for i in range(num):
                packet = Packet(flow, dst, flow.root, 0)
    ...
    def step(self, static=False):
        ...
        # self.compute_isl_distances()
        self.completed_flows.clear()
        rates = {1: np.random.poisson(90), 2: np.random.poisson(90), 3: np.random.poisson(90), 4: np.random.poisson(90)}
        input = 0
        ...
        for satellite in self.all_satellites:
            satellite.receive_packets()
            satellite.increment_life_time()
            satellite.step_c()
...
if __name__ == "__main__":
    ...
    env = Env(deployment, constellation)
    for i in range(4):
    ...
    for flow_id, delays in enumerate(avg_delay):
        # 剔除 inf 值
        valid_delays = [d for d in delays[: 3000] if not np.isinf(d)]
        # 计算平均值
